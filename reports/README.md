...
We unfortunately didn't use the cookiecutter template for our project. However we tried to follow the structure of the template as much as possible. The project structure can be seen in the README.md file in the repository. We have all the same files, except for the visualize.py file, which we didn't need. We also created new files for having an explanability element in the project and a part of our API front end, which is the explanation.py and frontend.py files in the src folder, this creates a saliency map for our frontend. We misnamed the evaluate.py file, and called it test.py file instead. Also, in our project, the file typically referred to as data.py is instead named datasets.py.
...
![image of buceket](figures/Buckets_overview.png)
...
![image of artifact registry](figures/artifact_register.png)
...
![image of Cloud build history](figures/Cloud_build_history.png)
...
We set up the training process to run locally first, using a dataset we uploaded to a Google Cloud bucket. The training code was modified to save the trained model to our model bucket. Once the process was confirmed to work locally, we prepared to run it on Vertex AI. To do this, we built a Docker image with the training process as the entry point and uploaded it to the Google Artifact Registry. We also created a configuration file to specify the necessary parameters and the Docker image to be used. If we had finalized the Dockerfile in time, we could have run the entire process on Vertex AI by executing a command like:
